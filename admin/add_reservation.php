...
<body>
   ...
   <main>
        <div class="container">
            ...
            <form action="search_routes.php" method="post">
    <!-- <div class="form-group">
    ...
    <div class="form-group">
        <label for="from_location">From Location</label>
        <select name="from_location" id="from_location" onchange="disableToLocation()" required  style="width:100%; height:30px;">
            <option value="">Select a Location</option>
            <?php
            $locationResult = $conn->query("SELECT id, city FROM location WHERE status = 1");
            while ($location = $locationResult->fetch_assoc()) {
                echo "<option value=\"{$location['city']}\">{$location['city']}</option>";
            }
            ?>
        </select>
    </div>
    ...
    <div class="form-group">
        <label for="to_location">To Location</label>
        <select name="to_location" id="to_location" required  style="width:100%; height:30px;">
            <option value="">Select a Location</option>
            <?php
            // Reload the locations to ensure the "To Location" list is complete
            $locationResult->data_seek(0); // Reset the result pointer
    ...
    <div class="form-group">
        <label for="departure_time">Departure Time</label>
        <input type="date" name="departure_time" id="departure_time" required style="width:100%; height:30px;">
    </div>
...
window.onload = function() {
    ...
    // Format the current date as "yyyy-MM-dd" (date format for input[type="date"])
    var currentDate = now.toISOString().split("T")[0];

    // Set the min attribute for departure_time input field to today's date
    document.getElementById("departure_time").setAttribute("min", currentDate); 

    // Style previous dates as gray and disable them in the calendar
    var inputField = document.getElementById("departure_time");
    
    // Add event listener to handle the min date change and disable previous dates in calendar
    inputField.addEventListener('input', function() {
        var selectedDate = new Date(inputField.value);
        if (selectedDate < now) {
            inputField.setCustomValidity("You cannot select a past date.");
        } else {

...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param("i", $id); // "i" means integer for bus id
...
if (!empty($successMessage)) {
    header("refresh:0.6;url=list_news.php"); // Redirect to list of buses
} elseif (!empty($errorMessage)) {
    echo "<p class='message error'>$errorMessage</p>";
    header("refresh:0.6;url=list_news.php"); // Redirect after showing error
}

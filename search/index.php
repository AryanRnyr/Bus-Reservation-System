...
<div style="position: relative; display: inline-block; width: 200px;">
    ...
    <div id="suggestions-arrival" class="arr-suggestions-list"></div>
                ...
                <input class="date" type="date" id="dateInput" name="date" min="<?= $date; ?>" 
    ...
    <?php while ($row = $result->fetch_assoc()): ?>
        <div class="bus-result">
            ...
            <div class="seats">
                ...
                <?php if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] === true): ?>
                    <button class="view-seats" onclick="window.location.href='../search/bus_details.php?schedule_id=<?= $row['schedule_id']; ?>&source=<?= urlencode($sourceCity); ?>&destination=<?= urlencode($destinationCity); ?>&date=<?= urlencode($selectedDate); ?>'">View Seats</button>
                    <?php else: ?>
    <?php
// Store the current bus page URL in the session
$_SESSION['redirect_to'] = '../search/bus_details.php?schedule_id=' . $row['schedule_id'] . '&source=' . urlencode($sourceCity) . '&destination=' . urlencode($destinationCity) . '&date=' . urlencode($selectedDate);
?>
    <button class="view-seats" onclick="window.location.href='../login/'">Login to View Seats</button>
<?php endif; ?>
